...
4개 모듈(Collector → Risk Analyzer/ML → Attack Path Engine → Dashboard)이 각자 설계 문서에 넣는 입출력 예시가 서로 다른 패키지/에이전트를 가정하면, 실제로 이어붙였을 때 안 맞을 수 있습니다. 그래서 아래 **하나의 가상 시나리오**를 전 모듈이 공유하는 기준 예시로 씁니다. 각 설계 문서(`docs/architecture_v1/design-0X-*.md`)의 "입출력 예시"는 가능하면 이 데이터를 그대로 인용하거나 확장해서 쓰세요.
