...
- 대표 에러 코드: `REPO_UNREACHABLE`, `REPO_PRIVATE_AUTH_REQUIRED`, `MANIFEST_PARSE_FAILED`(스킵), `IMPORT_SCAN_FAILED`(스킵), `PYPI_NOT_FOUND`(패키지 `resolution_status: "unresolved"`로 표시 후 계속), `PYPI_RATE_LIMIT_OR_5XX`(지수 백오프 최대 3회 후 unresolved), `DEPS_DEV_LOOKUP_FAILED`(2.6단계 실패 — `version_hint: null`로 두고 계속, 치명적 아님), `SBOM_GENERATION_FAILED`(스킵).
