...
| 필드 | 최소 요구사항 |
|---|---|
| 관련 이슈 | 맨 위 한 줄. `Closes #<이슈번호>`(완전히 끝남) 또는 `Refs #<이슈번호>`(관련만 있음) 중 하나 필수 |
...
| 변경 파일 | 변경한 파일/문서 경로를 전부 나열. `schemas/`가 있으면 CODEOWNERS 전원 리뷰 승인이 머지 조건(`CONTRIBUTING.md` §3) |
| 체크리스트 | 로컬 동작 확인 + 문서 업데이트 여부. 둘 다 체크 안 되어 있으면 머지 보류 |
| 검토 요청 | 필수 아님. 확신 없는 부분·설계 판단이 갈린 지점을 짚어줌 |
