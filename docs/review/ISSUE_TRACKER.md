...
전체 프로젝트는 GitHub Issue #1(Epic) 하나로 관리하고, 그 아래 R1~R4가 각자 상위 이슈를 갖습니다. 실제 작업 진행 상황은 각자 만드는 하위 이슈로 추적합니다.
...
- `OPEN` — 아직 완료조건을 다 못 채움
- `IN REVIEW` — 완료조건은 채웠고 리뷰어 승인 대기
- `CLOSED` — 리뷰 승인 완료

## 갱신 규칙

이슈 상태가 바뀌면(리뷰 요청/승인/종료) 이 표를 함께 갱신합니다. GitHub 이슈 자체가 진실 공급원이고, 이 표는 그걸 빠르게 훑기 위한 사본입니다.
